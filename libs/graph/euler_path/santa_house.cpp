...
int main()
{
    ...
    // House of Santa : http://ameblo.jp/masanori432/entry-10058793629.html
    ...
    if (!shand::graph::euler_path(g, vertex(E, g), [&path] (vertex_desc v) { path.push_front(v); })) {
        std::cout << "euler path failed" << std::endl;
        return 1;
    }

    BOOST_FOREACH (const vertex_desc& v, path) {
        std::cout << name[get(boost::vertex_index, g, v)] << std::endl;
    }
}
